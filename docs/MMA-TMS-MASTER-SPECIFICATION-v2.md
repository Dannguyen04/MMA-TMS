...
**Supersedes:** v1.0 (Baseline Specification)  
**Changes from v1.0:** Xem Section 27 (Change Log)
...
### Supported Actions (v2.0)

```
Kicks:
  - round_kick (đá vòng cầu)
  - front_kick (đá thẳng)
  - side_kick (đá ngang)
...
Punches:
  ...
  - cross
  - hook
  - uppercut (PROPOSED — phase 3)

Clinch/Grapple: (PROPOSED — phase 4)
  - takedown_attempt
  - clinch
```
...
├── TechniqueAnalyzer    (scoring rules per technique)
...
- **Nghiêm cấm:** Dùng từ "force" hoặc "impact force" với đơn vị Newton.
...
| NFR-AI-005 | Pose estimation accuracy baseline        | PCK@0.5 ≥ 70% on test set       |
| NFR-AI-006 | Action detection precision               | Precision ≥ 0.75, Recall ≥ 0.70 |
...
- Fighter profile
- Video upload → AI analysis (kick + punch)
- Analysis result: score, findings (fighter + coach language), evidence frames
