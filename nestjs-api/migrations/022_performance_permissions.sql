-- MMA-TMS 022: cấp quyền đọc tổng hợp hiệu suất theo phạm vi võ sĩ.
BEGIN;
SET LOCAL lock_timeout = '5s';
SET LOCAL statement_timeout = '30s';
SET LOCAL search_path = public, pg_catalog;
SELECT pg_catalog.pg_advisory_xact_lock(20260922, 22);
...
BEGIN
  IF to_regclass('public.permissions') IS NULL
    ...
    RAISE EXCEPTION '022 requires migration 003';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM mma_private.migration_history WHERE version = 21) THEN
    RAISE EXCEPTION '022 requires migration 021 history';
  END IF;
  IF EXISTS (SELECT 1 FROM mma_private.migration_history WHERE version = 22) THEN
    RAISE EXCEPTION '022 has already been applied';
  END IF;
  IF EXISTS (SELECT 1 FROM public.permissions WHERE code = 'performance:read') THEN
    RAISE EXCEPTION '022 performance permission collision';
  END IF;
END
$preflight$;
