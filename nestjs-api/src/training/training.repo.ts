...
function mapToSession(row: typeof trainingSessions.$inferSelect): TrainingSessionEntity {
...
export interface ITrainingRepository {
  ...
  findPlanById(id: string, database?: DatabaseExecutor): Promise<TrainingPlanEntity | undefined>;
  createPlan(data: CreatePlanInput, database?: DatabaseExecutor): Promise<TrainingPlanEntity>;
  updatePlan(id: string, data: UpdatePlanInput, database?: DatabaseExecutor): Promise<TrainingPlanEntity | undefined>;
  updatePlanStatus(
    id: string,
  ...
  getPlanProgress(planId: string, database?: DatabaseExecutor): Promise<{ totalSessions: number; completedSessions: number }>;
  ...
  findSessionById(id: string, database?: DatabaseExecutor): Promise<TrainingSessionEntity | undefined>;
  createSession(data: CreateSessionInput, database?: DatabaseExecutor): Promise<TrainingSessionEntity>;
  updateSession(id: string, data: UpdateSessionInput, database?: DatabaseExecutor): Promise<TrainingSessionEntity | undefined>;
  updateSessionStatus(
    id: string,
  ...
  // Exercises
  findExercises(query: ListExercisesQuery, database?: DatabaseExecutor): Promise<{ data: ExerciseEntity[]; total: number }>;
  findExerciseById(id: string, database?: DatabaseExecutor): Promise<ExerciseEntity | undefined>;
  createExercise(data: CreateExerciseInput, database?: DatabaseExecutor): Promise<ExerciseEntity>;
  updateExercise(id: string, data: UpdateExerciseInput, database?: DatabaseExecutor): Promise<ExerciseEntity | undefined>;

  // Plan Exercises
  findPlanExercises(planId: string, database?: DatabaseExecutor): Promise<TrainingPlanExerciseEntity[]>;
  findPlanExerciseById(id: string, database?: DatabaseExecutor): Promise<TrainingPlanExerciseEntity | undefined>;
  addPlanExercise(
    data: CreatePlanExerciseRecordInput,
    database?: DatabaseExecutor,
  ): Promise<TrainingPlanExerciseEntity>;
  updatePlanExercise(id: string, data: UpdatePlanExerciseInput, database?: DatabaseExecutor): Promise<TrainingPlanExerciseEntity | undefined>;
  removePlanExercise(
    id: string,
    database?: DatabaseExecutor,
  ): Promise<string | undefined>;
...
export class TrainingRepository implements ITrainingRepository {
  ...
  async findPlans(
    ...
    if (query.fighterId) conditions.push(eq(trainingPlans.fighterId, query.fighterId));
    if (query.coachId) conditions.push(eq(trainingPlans.coachId, query.coachId));
    if (query.status) conditions.push(eq(trainingPlans.status, query.status));
    if (query.isActive !== undefined) conditions.push(eq(trainingPlans.isActive, query.isActive));
    ...
    const [rows, countResult] = await Promise.all([
      database.select().from(trainingPlans).where(whereClause).orderBy(desc(trainingPlans.createdAt)).limit(query.limit).offset(offset),
      database.select({ count: sql<number>`count(*)::int` }).from(trainingPlans).where(whereClause),
    ]);
    return { data: rows.map(mapToPlan), total: countResult[0]?.count ?? 0 };
  }

  async findPlanById(id: string, database: DatabaseExecutor = this.db) {
    const rows = await database.select().from(trainingPlans).where(and(eq(trainingPlans.id, id), isNull(trainingPlans.deletedAt))).limit(1);
    return rows.length > 0 ? mapToPlan(rows[0]) : undefined;
  }

  async createPlan(data: CreatePlanInput, database: DatabaseExecutor = this.db) {
    const rows = await database.insert(trainingPlans).values({
  ...
  async getPlanProgress(planId: string, database: DatabaseExecutor = this.db) {
    const rows = await database
      ...
      .where(and(eq(trainingSessions.planId, planId), isNull(trainingSessions.deletedAt)));
  ...
  async findSessions(
    ...
    if (query.fighterId) conditions.push(eq(trainingSessions.fighterId, query.fighterId));
    if (query.coachId) conditions.push(eq(trainingSessions.coachId, query.coachId));
    if (query.planId) conditions.push(eq(trainingSessions.planId, query.planId));
    if (query.status) conditions.push(eq(trainingSessions.status, query.status));
    if (query.sessionType) conditions.push(eq(trainingSessions.sessionType, query.sessionType));
    if (query.fromDate) conditions.push(gte(trainingSessions.scheduledAt, new Date(query.fromDate)));
    if (query.toDate) conditions.push(lte(trainingSessions.scheduledAt, new Date(query.toDate)));
    if (scope.activeCoachId) {
      const now = new Date();
      conditions.push(
        exists(
    ...
    const [rows, countResult] = await Promise.all([
      database.select().from(trainingSessions).where(whereClause).orderBy(desc(trainingSessions.scheduledAt)).limit(query.limit).offset(offset),
      database.select({ count: sql<number>`count(*)::int` }).from(trainingSessions).where(whereClause),
    ]);
  ...
  async findSessionById(id: string, database: DatabaseExecutor = this.db) {
    const rows = await database.select().from(trainingSessions).where(and(eq(trainingSessions.id, id), isNull(trainingSessions.deletedAt))).limit(1);
    return rows.length > 0 ? mapToSession(rows[0]) : undefined;
  }

  async createSession(data: CreateSessionInput, database: DatabaseExecutor = this.db) {
    const rows = await database.insert(trainingSessions).values({
      fighterId: data.fighterId,
  ...
  async updateSession(id: string, data: UpdateSessionInput, database: DatabaseExecutor = this.db) {
    if (Object.keys(data).length === 0) return this.findSessionById(id, database);
    ...
    const rows = await database.update(trainingSessions).set(updateData).where(and(eq(trainingSessions.id, id), isNull(trainingSessions.deletedAt))).returning();
    return rows.length > 0 ? mapToSession(rows[0]) : undefined;
  }

  async updateSessionStatus(id: string, expectedStatus: SessionStatusType, newStatus: SessionStatusType, time: Date, outcome: SessionTransitionOutcome, database: DatabaseExecutor = this.db) {
    const updateData: Partial<typeof trainingSessions.$inferInsert> = {
      status: newStatus,
      completedAt: null,
      cancelledAt: null,
  ...
  async findExercises(query: ListExercisesQuery, database: DatabaseExecutor = this.db) {
    const conditions = [isNull(exercises.deletedAt)];
    if (query.category) conditions.push(eq(exercises.category, query.category));
    if (query.targetMuscle) conditions.push(arrayContains(exercises.targetMuscleGroups, [query.targetMuscle]));
    ...
    const [rows, countResult] = await Promise.all([
      database.select().from(exercises).where(whereClause).orderBy(desc(exercises.createdAt)).limit(query.limit).offset(offset),
      database.select({ count: sql<number>`count(*)::int` }).from(exercises).where(whereClause),
    ]);
  ...
  async findExerciseById(id: string, database: DatabaseExecutor = this.db) {
    const rows = await database.select().from(exercises).where(and(eq(exercises.id, id), isNull(exercises.deletedAt))).limit(1);
    return rows.length > 0 ? mapToExercise(rows[0]) : undefined;
  }

  async createExercise(data: CreateExerciseInput, database: DatabaseExecutor = this.db) {
    const rows = await database.insert(exercises).values({
      name: data.name,
      description: data.description ?? null,
  ...
  async updateExercise(id: string, data: UpdateExerciseInput, database: DatabaseExecutor = this.db) {
    if (Object.keys(data).length === 0) return this.findExerciseById(id, database);
    const updateData: Partial<typeof exercises.$inferInsert> = {
      ...data,
      updatedAt: new Date(),
    };
    const rows = await database.update(exercises).set(updateData).where(and(eq(exercises.id, id), isNull(exercises.deletedAt))).returning();
    return rows.length > 0 ? mapToExercise(rows[0]) : undefined;
  }

  // --- Plan Exercises ---

  async findPlanExercises(planId: string, database: DatabaseExecutor = this.db) {
    const rows = await database.select().from(trainingPlanExercises).where(eq(trainingPlanExercises.planId, planId)).orderBy(trainingPlanExercises.orderIndex);
    return rows.map(mapToPlanExercise);
  }

  async findPlanExerciseById(id: string, database: DatabaseExecutor = this.db) {
    const rows = await database.select().from(trainingPlanExercises).where(eq(trainingPlanExercises.id, id)).limit(1);
  ...
  async addPlanExercise(
    ...
    const rows = await database.insert(trainingPlanExercises).values({
      planId: data.planId,
  ...
  async updatePlanExercise(id: string, data: UpdatePlanExerciseInput, database: DatabaseExecutor = this.db) {
    if (Object.keys(data).length === 0) {
      const rows = await database.select().from(trainingPlanExercises).where(eq(trainingPlanExercises.id, id)).limit(1);
      return rows.length > 0 ? mapToPlanExercise(rows[0]) : undefined;
    }
    const updateData: Partial<typeof trainingPlanExercises.$inferInsert> = { ...data };
    const rows = await database.update(trainingPlanExercises).set(updateData).where(eq(trainingPlanExercises.id, id)).returning();
    return rows.length > 0 ? mapToPlanExercise(rows[0]) : undefined;
  }
