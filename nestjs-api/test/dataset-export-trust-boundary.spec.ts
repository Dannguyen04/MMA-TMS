...
describeWithDatabase(
  'Task 14 — Product Backend Security & Governance Remediation Gate',
  () => {
    ...
    const seedReviews = async (
      exportId: string,
      reviews: Array<{ reviewerId: string; role: string; status?: string }>,
    ) => {
    ...
    const seedQuality = async (
      ...
      policyVersion: string = 'quality_policy_v2.0',
    ) => {
    ...
    const samplePayload = (
      candidateOverride: Record<string, any> = {},
      ...
      const reviewData = {
        ...
        reviewers: reviewEvidence.reviewers
          .map((r: any) => ({ reviewerId: r.reviewerId, role: r.role }))
          .sort((a: any, b: any) => a.reviewerId.localeCompare(b.reviewerId)),
      };
      const qualityData = {
      ...
      const reviewEvidenceDigest =
        candidateOverride.reviewEvidenceDigest || computeDigest(reviewData);
      const qualityEvidenceDigest =
        candidateOverride.qualityEvidenceDigest || computeDigest(qualityData);

      const exportId =
        candidateOverride.exportId ||
        `exp_adv_${Date.now()}_${Math.floor(Math.random() * 100000)}`;
      const datasetHash =
        candidateOverride.datasetHash || 'sha256:' + 'a'.repeat(64);
      const policyVersion =
        candidateOverride.policyVersion || 'dataset-policy-v1';
      const sourceSchemaVersion =
        candidateOverride.sourceSchemaVersion || '1.0.0';
      const sampleCount =
        candidateOverride.sampleCount !== undefined
          ? candidateOverride.sampleCount
          : 600;
      const coveredActionIdsHash =
        candidateOverride.coveredActionIdsHash || 'sha256:' + 'e'.repeat(64);
      ...
      const manifestDigest =
        candidateOverride.manifestDigest || computeDigest(manifestData);
    ...
    beforeEach(async () => {
      ...
      signerService = moduleFixture.get<AttestationSignerService>(
        AttestationSignerService,
      );
      governanceService = moduleFixture.get<AuthoritativeGovernanceService>(
    ...
    describe('1. Production Fail-Closed Defaults & Authorization Guard', () => {
      ...
      it('1.2 rejects development secret when NODE_ENV=production', async () => {
        ...
        const res = await request(app.getHttpServer())
          .post(
            `/internal/dataset-exports/${payload.candidate.exportId}/promotions`,
          )
      ...
      it('1.3 derives actor identity from authentication context, rejecting body actorId spoofing', async () => {
        const spoofedPayload = samplePayload(
          {},
          { actorId: 'spoofed_admin_hacker' },
        );
        const resSpoofed = await request(app.getHttpServer())
          .post(
            `/internal/dataset-exports/${spoofedPayload.candidate.exportId}/promotions`,
          )
        ...
        const resValid = await request(app.getHttpServer())
          .post(
            `/internal/dataset-exports/${validPayload.candidate.exportId}/promotions`,
          )
          .set('x-worker-secret', validWorkerSecret)
    ...
    describe('2. Authoritative Governance & Digest Binding', () => {
      it('2.1 fails closed as NOT_GOLD_READY when DB has no authoritative reviews or quality report', async () => {
        ...
        expect(
          res.body.readinessGaps.some((g: string) =>
            g.includes('MISSING_AUTHORITATIVE_REVIEWS'),
          ),
        ).toBe(true);
        expect(
          res.body.readinessGaps.some((g: string) =>
            g.includes('MISSING_AUTHORITATIVE_QUALITY_REPORT'),
          ),
        ).toBe(true);
      });
      ...
      it('2.3 rejects dual review consensus when authoritative DB reviewer role combination is invalid', async () => {
        ...
        expect(res.status).toBe(201);
        expect(res.body.authoritativeStatus).toBe('NOT_GOLD_READY');
        expect(res.body.readinessGaps.join(' ')).toContain(
          'exact required role combination',
        );
      });
      ...
      it('2.4 fails closed when a reviewer is revoked in DB reducing count below threshold', async () => {
        ...
        expect(res.body.authoritativeStatus).toBe('NOT_GOLD_READY');
        expect(res.body.readinessGaps.join(' ')).toContain(
          'Dual review consensus requires at least 2 distinct reviewers',
        );
      });
      ...
      it('2.6 flags reviewEvidenceDigest mismatch when candidate claim does not match authoritative DB', async () => {
        const payload = samplePayload({
          reviewEvidenceDigest: 'sha256:' + 'f'.repeat(64),
        });
        const exportId = payload.candidate.exportId;
    ...
    describe('3. Candidate Conflict & Superseding Lifecycle', () => {
      it('3.1 rejects same exportId submitted with different candidate hash (409 Conflict)', async () => {
        ...
        const payload2 = samplePayload({
          exportId,
          datasetHash: 'sha256:' + '9'.repeat(64),
        });
      ...
      it('3.2 supersedes previous active attestations when issuing a new promotion', async () => {
        ...
        expect(statusRes.body.attestation.attestationId).toBe(
          secondAttestationId,
        );
      }, 15000);
    });
    ...
    describe('4. Idempotency & Nonce Replay', () => {
      ...
      it('4.3 rejects duplicate nonces with 409 Conflict', async () => {
        ...
        await request(app.getHttpServer())
          .post(
            `/internal/dataset-exports/${payload1.candidate.exportId}/promotions`,
          )
          .set('x-worker-secret', validWorkerSecret)
        ...
        const res2 = await request(app.getHttpServer())
          .post(
            `/internal/dataset-exports/${payload2.candidate.exportId}/promotions`,
          )
          .set('x-worker-secret', validWorkerSecret)
    ...
    describe('5. Cryptographic Verification & Malformed Input Safety', () => {
      it('5.1 returns false safely on malformed signature verification without throwing', () => {
        ...
        const result = signerService.verifySignature(
          claims,
          'invalid_hex_sig!!!',
          'wrong_key_id',
        );
      ...
      it('5.2 verifies cross-language RFC 8785 canonical JSON fixture byte parity', () => {
        const fixturePath = path.join(
          __dirname,
          'fixtures',
          'canonical-json-fixture.json',
        );
        const fixtureData = JSON.parse(fs.readFileSync(fixturePath, 'utf8'));

        for (const caseData of fixtureData.testCases) {
          const canonicalStr = governanceService.canonicalizeJson(
            caseData.input,
          );
