"""
reference_normalization.py — Reference Motion Normalization (Task 29 & TL-07)
...
- Skeleton trajectory normalization for Ghost Mode & reference alignment:
  ...
  * Explicit stance validation: requires explicit stance (orthodox or southpaw); never falls back to defaults.
  * Explicit camera view metadata support.
  * Unified normalization for both reference and athlete frames into identical coordinate space.
...
class ReferenceNormalizer:
    ...
    def normalize_single_frame(
        frame_kps: Sequence[tuple[float, float]],
        should_mirror: bool = False,
    ) -> tuple[tuple[float, float], ...]:
        """
        Normalizes a single skeleton frame to mid-hip origin and torso scale.
        Returns empty tuple if frame is invalid or degenerate.
        """
        if len(frame_kps) < 13:
            return ()
        ...
        return tuple(norm_kps_list)

    def normalize_athlete_frames(
        self,
        ...
        """
        Normalizes athlete frames using the exact same coordinate system and version as reference.
        """
        norm_s = normalize_stance(stance)
        if norm_s is None:
            raise ValueError(f"Explicit athlete stance ('orthodox' or 'southpaw') is required, got {stance!r}")

        norm_target = normalize_stance(target_stance) if target_stance else norm_s
        should_mirror = (norm_target is not None and norm_s != norm_target)

        return [
            self.normalize_single_frame(f_kps, should_mirror=should_mirror)
            for f_kps in raw_frames
        ]
    ...
    def normalize_trajectory(
        ...
        norm_src = normalize_stance(source_stance)
        norm_tgt = normalize_stance(target_stance)
        if norm_src is None or norm_tgt is None:
            raise ValueError(
                f"Explicit stance ('orthodox' or 'southpaw') is required for both source and target. "
                f"Got source_stance={source_stance!r}, target_stance={target_stance!r}"
            )

        should_mirror = (norm_src != norm_tgt)

        normalized_frames: list[NormalizedFrameSkeleton] = []

        for f_idx, frame_kps in enumerate(raw_frames):
            norm_kps = self.normalize_single_frame(frame_kps, should_mirror=should_mirror)
            if not norm_kps:
        ...
        if camera_view is not None:
            digest_dict["cameraView"] = camera_view

        digest_input = json.dumps(digest_dict, sort_keys=True)
